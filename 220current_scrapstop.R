# Define all the different scenarios from DNR to alpha=1
C_R_values <- c(5450.866667, 8176.3, 8721.386667,	9266.473333,	9811.56,	10356.64667,	10901.73333,
                5450.866667, 8176.3, 8721.386667,	9266.473333,	9811.56,	10356.64667,	10901.73333,
                5450.866667, 8176.3, 8721.386667,	9266.473333,	9811.56,	10356.64667,	10901.73333)
p_values <- c(1.3, 1.3, 1.3, 1.3, 1.3, 1.3, 1.3, 
              1.59, 1.59,1.59,1.59,1.59,1.59,1.59,
              1.91, 1.91,1.91,1.91,1.91,1.91,1.91)
phi_values <- c(4886.377591,
                7939.702674,
                9460.626752,
                11280.73836,
                13713.15804,
...
N_0 = 4400 #20 per tree, 220 trees, 10% of K
...
for (scenario in 1:length(phi_values)) {
  
  # Update the ab and C_R parameters
  C_R <- C_R_values[scenario]
  p <- p_values[scenario]
  phi <- phi_values[scenario]
  ...
  results <- data.frame(initial_age = integer(), ab = numeric(), C_R = numeric(), p = numeric(), NPV = numeric(), T = integer(), S = integer())
  ...
  for (initial_age in 1:20) {
    ...
    for (S in 1:T+1) {
      maxS_NPV <- -Inf #for each S, we need to find the T that max npv
      optimalS_T <- 0
      # time loop
      for (t in 1:T) {
        # Calculate the initial population sizes
      ...
      for (ii in 1:T) {
        ...
        # profit <-revenue- C_I * ii - C_M * ii - C_R
        
        # Modify the equation for profits
        profits[ii] <- totalrevenue[ii]- C_I * (S-1)- C_f * ii - C_M * ii - C_R
        # Calculate the Net Present Value (NPV)
        
        NPV <- (profits[ii]+ phi)/ ((1 + r)^ii)
        
        # npvs[t] <-  (profits[t])/ ((1 + r)^t)
        if (NPV > maxS_NPV) {
          maxS_NPV <- NPV
          optimalS_T <- ii #this is the optimal t for that S
        }
      }
      if (maxS_NPV > max_NPV) {
        max_NPV <- maxS_NPV
        optimal_T <- optimalS_T
        optimal_S <- S
      }
      # NPVS_A[, initial_age] <- npvs
      # PROFITS_A[, initial_age] <- profits
      # Revenue_A[, initial_age] <- totalrevenue
      # YIELDS_A[, initial_age] <- totalyield
    }
    # Add the maximum NPV and the optimal T and S for the current scenario and price to the results data frame
    results <- rbind(results, data.frame(a = initial_age, ab = ab, C_R = C_R, p = p, NPV = max_NPV, T = optimal_T, S = optimal_S))
    
    # results <- rbind(results, data.frame(a = initial_age, NPV = max_NPV, T = optimal_T, S = optimal_S))
    # }
    # pricescenarios[[iii]]<- results
  }
  list_of_allresults[[scenario]]<-results
}

# Save the list_of_allresults to a .rds file
saveRDS(list_of_allresults, file = "C:/Users/juanj/Desktop/RA/Research/citrus supply chain/modeling/results/NPV_scrapstop220.rds")
# Read the list_of_allresults from a .rds file
list_of_allresults <- readRDS("C:/Users/juanj/Desktop/RA/Research/citrus supply chain/modeling/results/NPV_scrapstop220.rds")
...
write.csv(flat_df, file = "C:/Users/juanj/Desktop/RA/Research/citrus supply chain/modeling/results/NPV_scrapstop220.csv")
